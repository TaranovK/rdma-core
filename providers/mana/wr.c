...
gdma_get_current_wqe(struct mana_gdma_queue *wq, uint32_t client_oob_size,
		     uint32_t num_sge, struct gdma_wqe *wqe)
{
	uint32_t wqe_size = align(sizeof(union gdma_oob) + client_oob_size +
				  sizeof(struct gdma_sge) * num_sge, GDMA_WQE_ALIGNMENT_UNIT_SIZE);
...
static inline void gdma_write_sge(struct gdma_wqe *wqe, struct ibv_sge *oob_sge,
				  struct ibv_sge *sge, uint32_t num_sge)
{
	...
	if (oob_sge) {
		gdma_sgl->address = oob_sge->addr;
		gdma_sgl->size = oob_sge->length;
		gdma_sgl->mem_key = oob_sge->lkey;
		gdma_sgl++;
		num_sge1--;
	}
...
static inline int
gdma_post_rq_wqe(struct mana_gdma_queue *wq, struct ibv_sge *sgl,  struct rdma_recv_oob *oob,
		 uint32_t num_sge, enum gdma_work_req_flags flags, struct gdma_wqe *wqe)
{
	int ret;

	ret = gdma_get_current_wqe(wq, INLINE_OOB_SMALL_SIZE, num_sge, wqe);
	if (ret)
		return ret;
	...
	if (oob)
		memcpy(wqe->client_oob, oob, sizeof(*oob));
...
gdma_post_sq_wqe(struct mana_gdma_queue *wq, struct ibv_sge *sgl, struct rdma_send_oob *send_oob,
		 struct ibv_sge *oob_sge,  uint32_t num_sge, uint32_t mtu,
		 enum gdma_work_req_flags flags, struct gdma_wqe *wqe)
{
	...
	ret = gdma_get_current_wqe(wq, INLINE_OOB_LARGE_SIZE, total_sge, wqe);
	if (ret)
		return ret;

	wqe->gdma_oob->tx.num_sgl_entries = total_sge;
	wqe->gdma_oob->tx.inline_client_oob_size = INLINE_OOB_LARGE_SIZE / sizeof(uint32_t);
	wqe->gdma_oob->tx.client_oob_in_sgl = (oob_sge ? 1 : 0);
	...
	memcpy(wqe->client_oob, send_oob, sizeof(*send_oob));
			     ...
			     uint32_t *unmasked_wqe_index)
{
	...
	if (wr->opcode == IBV_WR_RDMA_READ) {
		...
		read_size_in_bu = gdma_wqe.size_in_bu;
	...
	case IBV_WR_SEND_WITH_INV:
		...
		break;
	case IBV_WR_SEND_WITH_IMM:
		send_oob.send.immediate = wr->imm_data;
		SWITCH_FALLTHROUGH;
	case IBV_WR_SEND:
		flags |= GDMA_WORK_REQ_CHECK_SN;
		break;
	...
	ret = gdma_post_sq_wqe(&qp->rc_qp.queues[USER_RC_SEND_QUEUE_REQUESTER], wr->sg_list,
			       &send_oob, NULL, num_sge, MTU_SIZE(qp->mtu), flags, &gdma_wqe);
	if (ret) {
		verbs_err(verbs_get_ctx(qp->ibqp.qp.context),
	...
	qp->rc_qp.acc_posted_wqe_size += gdma_wqe.size_in_bu;
	qp->rc_qp.acc_read_posted_wqe_size += read_size_in_bu;
	qp->rc_qp.sq_psn = PSN_ADD(qp->rc_qp.sq_psn, PSN_DELTA(msg_sz, qp->mtu));
	qp->rc_qp.sq_ssn = PSN_INC(qp->rc_qp.sq_ssn);

	*unmasked_wqe_index = gdma_wqe.unmasked_wqe_index;
				...
				struct ibv_send_wr **bad_wr)
{
	...
	for (; wr; wr = wr->next) {
		...
		ret = mana_ib_rc_post_send_request(qp, wr, &unmasked_queue_offset);
		if (ret) {
			verbs_err(verbs_get_ctx(qp->ibqp.qp.context),
		...
		if (wr->send_flags & IBV_SEND_SIGNALED) {
			...
			shadow_wqe->header.posted_wqe_size_in_bu = qp->rc_qp.acc_posted_wqe_size;
			shadow_wqe->header.unmasked_queue_offset = unmasked_queue_offset;
			shadow_wqe->end_psn = PSN_DEC(qp->rc_qp.sq_psn);
			shadow_wqe->read_posted_wqe_size_in_bu =
				qp->rc_qp.acc_read_posted_wqe_size;
			shadow_queue_advance_producer(&qp->shadow_sq);

			qp->rc_qp.acc_read_posted_wqe_size = 0;
			qp->rc_qp.acc_posted_wqe_size = 0;
		}
